...
def carre_magique(mat: List[List[int]]) -> bool:
    dim = len(mat)
...
def carre_magique_normal(mat: List[List[int]]) -> bool:
    liste_chiffres = [i for i in range(1, 1+ len(mat) ** 2)]
    print(liste_chiffres)
    for ligne in mat:
        for chiffre in ligne:
            if chiffre not in liste_chiffres:
                return False
            liste_chiffres.remove(chiffre)
    if liste_chiffres:
        return False
    return carre_magique(mat)
...
def main():
    ...
    print(f"magique : {carre_magique(mat1)}")

    print(
        f"Carré magique normal : {carre_magique_normal([[8, 1, 6], [3, 5, 7], [4, 9, 2]])}"
    )
    print(
        f"Carré magique normal : {carre_magique_normal([[21, 7, 17], [11, 15, 19], [13, 23, 9]])}"
    )
